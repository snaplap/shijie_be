...
public class AuthService {
    ...
    public SendCodeResponse sendCode(SendCodeRequest request) {
        ...
        if ((request.scene() == VerificationScene.LOGIN || request.scene() == VerificationScene.RESET_PASSWORD)
                && !exists) {
    ...
    public AuthResponse register(RegisterRequest request, ClientInfo clientInfo) {
        ...
        loginLogService.record(user.getId(), identifier, "REGISTER", clientInfo.ip(), clientInfo.userAgent(),
                "SUCCESS");
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        ...
        if (StringUtils.hasText(request.password())) {
            ...
            if (!StringUtils.hasText(user.getPasswordHash())
                ...
                loginLogService.record(user.getId(), identifier, channel, clientInfo.ip(), clientInfo.userAgent(),
                        "FAILED");
    ...
    private TokenResponse mapToken(TokenPair tokenPair) {
        return new TokenResponse(tokenPair.accessToken(), tokenPair.accessTokenExpiresAt(), tokenPair.refreshToken(),
    ...
    private String generateNickname() {
        return "识界用户" + UUID.randomUUID().toString().substring(0, 8);
    }
